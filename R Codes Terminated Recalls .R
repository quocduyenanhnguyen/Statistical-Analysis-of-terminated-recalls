...
#import data
recalls_df = read.csv("Documents/BUS2_194A_Statistical_Analysis/Group_Project/terminated_recalls.csv")
...
recallsEDA_df = read.csv("Documents/BUS2_194A_Statistical_Analysis/Group_Project/recallsEDA.csv") 
# Clean the data for 2021
recallsEDA1_df<- subset(recallsEDA_df, select = X2021)
recallsEDA1_df
recallsEDA1_df <- na.omit(recallsEDA1_df)
recallsEDA1_df
# Clean data for 2020 
recallsEDA2_df<- subset(recallsEDA_df, select = X2020)
recallsEDA2_df
...
xtabs(~X2021 + X2020, data = recalls_df) #Creates a two-way contingency table of categorical outcome and predictors
hist(recallsEDA1_df$X2021) #histogram 
hist(recallsEDA2_df$X2020)

#pie chart for 2021 
recallsEDApie_df = read.csv("Documents/BUS2_194A_Statistical_Analysis/Group_Project/recallsEDApie.csv") 
labels = c("No", "Yes")
piepercent<- round(100 * recallsEDApie_df$X2021 / sum(recallsEDApie_df$X2021), 1)
pie(recallsEDApie_df$X2021,labels = piepercent, main = "Percentage of terminated recalls 2021", col = rainbow(length(recallsEDApie_df$X2021)))
legend("topright", c("No","Yes"), cex = 0.5, fill = rainbow(length(recallsEDApie_df$X2021)))
#pie chart for 2020
piepercent2<- round(100 * recallsEDApie_df$X2020 / sum(recallsEDApie_df$X2020), 1)
pie(recallsEDApie_df$X2020,labels = piepercent2, main = "Percentage of terminated recalls 2020", col = rainbow(length(recallsEDApie_df$X2020)))
legend("topright", c("No","Yes"), cex = 0.5, fill = rainbow(length(recallsEDApie_df$X2020)))

#Hypothesis test
# Clean the data for 2021
recalls2021_df<- subset(recalls_df, select = X2021)
recalls2021_df
...
yes2021 <- subset(recalls2021_df, X2021=="Yes")
yes2021
...
no_2021 <- subset(recalls2021_df, X2021=="No")
no_2021
...
# Clean data for 2020 
recalls2020_df<- subset(recalls_df, select = X2020)
recalls2020_df
...
# Look at the number of Yes in the data
yes2020 <- subset(recalls2020_df, X2020 =="Yes")
yes2020
...
# Look at the number of No in the data
no_2020 <- subset(recalls2020_df, X2020=="No")
no_2020
